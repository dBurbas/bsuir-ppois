...
TEST_F(CantorSetFixture, AddElementInvalidAtomic) {
    ASSERT_FALSE(test_set.AddElement("ab"));
    ASSERT_FALSE(test_set.AddElement("@"));
}
...
TEST_F(CantorSetFixture, EraseElementInvalid) {
    ASSERT_FALSE(test_set.EraseElement("ab"));
    ASSERT_FALSE(test_set.EraseElement("@"));
}
...
TEST(CantorSetNonFixture, PowerSetTooLarge) {
    CantorSet set("{a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u}");
