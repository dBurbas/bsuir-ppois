...
public:
    ...
    friend std::istream& operator<<(std::ostream out_stream, BinarySearchTree& tree);
...
private:
    ...
    Node* ClearHelper(Node* node);
    
    Node* CopyHelper(const Node* node);
    
    void PrintInOrderHelper(const Node* node) const;
    
    Node* EraseHelper(Node* node, const KeyValue& key, bool& erased);
